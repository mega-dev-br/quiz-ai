{{-- resources/views/livewire/quiz-show.blade.php --}}
  ...
  <div
    ...
    <header class="mb-6">
      ...
      <div class="w-full h-2 bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden">
        <div class="h-2 bg-blue-600 rounded-full transition-all duration-300"
          style="width: {{ intval(($currentIndex+1)/count($questions)*100) }}%">
        </div>
      </div>
    ...
    <section class="mb-6">
      ...
      <div class="space-y-3">
        ...
        <label wire:click="selectOption({{ $opt['id'] }})" class="flex items-center space-x-3 p-3 bg-zinc-50 dark:bg-zinc-700 border dark:border-zinc-600 rounded-lg cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-600
                   {{ (isset($answers[$currentIndex]) && $answers[$currentIndex]==$opt['id'])
                       ? 'border-blue-600 bg-blue-50 dark:bg-blue-900' : '' }}">
          <input type="radio" name="answer_{{ $currentIndex }}"
            class="h-5 w-5 text-blue-600 focus:ring-2 focus:ring-blue-400"
            {{ (isset($answers[$currentIndex]) && $answers[$currentIndex]==$opt['id']) ? 'checked' : '' }} />
